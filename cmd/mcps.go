	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		res, raw, err := apiClient.CreateMcp(
			...
			platform.McpCreateRequest{
				Name:        mcpName,
				Backend:     backend,
				Description: strPtr(mcpDescription),
...
to it. Auth routing cannot change while agents are attached — detach them first.`,
  ...
  iai mcps update my-tool --description "notes for the team"`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := requireWholeWorkload(cmd); err != nil {
			return err
		}
...
func requireWholeWorkload(cmd *cobra.Command) error {
	passed := make([]string, 0, len(workloadFlags))
	...
	for _, name := range []string{"image-name", "image-tag"} {
		if !cmd.Flags().Changed(name) {
			return fmt.Errorf(
				"%s replaces the mcp's whole workload, so --image-name and --image-tag are required with it; pass --port, --path, --memory and --cpu too, or they return to their defaults",
				strings.Join(passed, ", "),
			)
		}
	}
