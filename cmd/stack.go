...
The organization and project are read from the config file or resolved via 'iai organizations select' / 'iai projects select'.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		orgName, err := sess.ResolveOrganizationFromConfig(cfg.Organization)
		if err != nil {
			return err
		}

		projectName, err := sess.ResolveProjectFromConfig(cfg.Project)
		if err != nil {
			return err
		}
		...
		if len(cfg.Services) > 0 {
			...
			svcResult, err := deployClient.SyncServices(
				cmd.Context(),
				orgId,
				projectId,
				cfg.StackId,
		...
		if len(cfg.VectorStores) > 0 {
			...
			vsResult, err := deployClient.SyncVectorStores(
				cmd.Context(),
				orgId,
				projectId,
				cfg.StackId,
