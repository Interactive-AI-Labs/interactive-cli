...
// ResolveOrganizationIDByName looks up an organization by name using the
// /api/v1/session/organizations endpoint and returns its ID.
//
// It requires a valid session (stored via SaveSessionCookies). If no
// organization with the given name exists, or the name is ambiguous, it
// returns an error. Matching is case-insensitive.
//
// Parameters:
//   - ctx: request context (if nil, context.Background is used).
//   - apiHostname: base API hostname, e.g. "https://app.interactive.ai".
//   - cfgDirName, sessionFileName: passed through to LoadSessionCookies.
//   - orgName: organization name to resolve.
//   - timeout: HTTP client timeout.
func ResolveOrganizationIDByName(
	ctx context.Context,
	apiHostname string,
	cfgDirName string,
...
// ResolveProjectIDByName looks up a project by name within an organization,
// using the organizations and projects session APIs. It returns the resolved
// organization ID and project ID.
func ResolveProjectIDByName(
	ctx context.Context,
	apiHostname string,
	cfgDirName string,
	sessionFileName string,
	...
	orgID, err := ResolveOrganizationIDByName(ctx, apiHostname, cfgDirName, sessionFileName, orgName, timeout)
	if err != nil {
		return "", "", err
	}
