...
func TestPrintPromptDiff(t *testing.T) {
	tests := []struct {
		name     string
		versionA string
		a        *platform.PromptDetail
		versionB string
		b        *platform.PromptDetail
		want     string
	}{
		{
			name:     "identical content shows no differences",
			versionA: "1",
			a:        &platform.PromptDetail{Prompt: json.RawMessage(`"Hello world"`)},
			versionB: "2",
			b:        &platform.PromptDetail{Prompt: json.RawMessage(`"Hello world"`)},
		...
		},
		{
			...
			a:        &platform.PromptDetail{Prompt: json.RawMessage(`"Hello world"`)},
			versionB: "2",
			b:        &platform.PromptDetail{Prompt: json.RawMessage(`"Hello universe"`)},
			want: "--- version 1\n" +
				"+++ version 2\n" +
				"@@ -1,1 +1,1 @@\n" +
				"-Hello world\n" +
