...
func TestPrintSessionSummary(t *testing.T) {
	cases := []struct {
				...
				Turns: []summary.Turn{
					{
						...
						Tools:    []string{"check_availability"},
					},
					{
		...
		{
			name:  "empty session shows empty-state message",
			model: &summary.SessionSummaryModel{ID: "s_x"},
			want:  []string{"No turns found."},
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := PrintSessionSummary(&buf, tc.model); err != nil {
				t.Fatal(err)
			}
			got := buf.String()
