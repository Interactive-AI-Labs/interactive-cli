...
func validateTimestamp(value, name string) error {
	...
	if _, err := time.Parse(time.RFC3339, value); err != nil {
		return fmt.Errorf("invalid %s %q: must be ISO 8601 format (e.g. 2025-01-01T00:00:00Z)", name, value)
	}
	return nil
}
...
func validateOrderBy(value string) error {
	...
	if len(parts) != 2 {
		return fmt.Errorf("invalid order-by %q: must be field.direction (e.g. timestamp.desc)", value)
	}
