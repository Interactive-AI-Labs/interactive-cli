...
var validOrderByFields = []string{"id", "name", "timestamp", "userId", "sessionId", "release", "version", "public"}
...
func ValidateTraceListOptions(opts clients.TraceListOptions) error {
	...
	if opts.FromTimestamp != "" {
		if _, err := time.Parse(time.RFC3339, opts.FromTimestamp); err != nil {
			return fmt.Errorf("invalid from-timestamp %q: must be ISO 8601 format (e.g. 2025-01-01T00:00:00Z)", opts.FromTimestamp)
		}
	}
	if opts.ToTimestamp != "" {
		if _, err := time.Parse(time.RFC3339, opts.ToTimestamp); err != nil {
			return fmt.Errorf("invalid to-timestamp %q: must be ISO 8601 format (e.g. 2025-01-01T00:00:00Z)", opts.ToTimestamp)
		}
	}
	if opts.OrderBy != "" {
		parts := strings.Split(opts.OrderBy, ".")
		if len(parts) != 2 {
			return fmt.Errorf("invalid order-by %q: must be field.direction (e.g. timestamp.desc)", opts.OrderBy)
		}
		if !slices.Contains(validOrderByFields, parts[0]) {
			return fmt.Errorf("invalid order-by field %q (available: %s)", parts[0], strings.Join(validOrderByFields, ", "))
		}
